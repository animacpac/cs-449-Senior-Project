...
class LoginRegister():
    ...
    def registration_screen(self):
        ...
        # business
        business_register_entry = tk.Entry(frame, width=20, textvariable=business, font=('Verdana', 25), bd=1,
                                           relief='solid')
        business_register_entry.placeholder = 'Business description'

        business_register_entry.insert(0, business_register_entry.placeholder)
        business_register_entry.bind('<FocusIn>', lambda event: on_entry_click(event, business_register_entry))
        business_register_entry.bind('<FocusOut>', lambda event: on_focusout(event, business_register_entry,
                                                                             business_register_entry.placeholder))
        business_register_entry.config(fg='grey')
        business_register_entry.pack(pady=(0, 50))  # Only applies padding on bottom on vertical axis
        ...
        def register_user():
            ...
            business = business_register_entry.get()
            ...
            if len(password) < 8 or password.islower() or username == 'Username' or password == 'Password':
                ...
                business_register_entry.pack_forget()
            ...
            # Grabbing all the things and finally registring an user
                ...
                if my_data == []:
                    ...
                    if response:
                        ...
                        business_register_entry.delete(0,len(business))
                        password_match_register_entry.delete(0, tk.END)
